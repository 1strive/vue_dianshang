...
4:需要知道连续页码数【起始数字、结束数字：连续页码个数市场当中一般5、7、9】奇数（因为对称），对称好看 continues：代表分页连续页码个数
...
总结:
面试问题：v-for与v-if优先级?(不能同时使用（可以嵌套使用，v-if为父元素， v-for为子元素）)

 在`Vue`处理时，`v-for`的优先级高于`v-if`，所以不管你`condition`是否成立，`v-for`都会做循环,所以是达不到先判断再循环的作用.       
...
v-for可以遍历number string object Array
